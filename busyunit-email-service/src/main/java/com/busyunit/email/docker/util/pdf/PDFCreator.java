...
public class PDFCreator {
    ...
    public static void addMetaData(Document document, String sqlXMLFileName) {
        ...
        document.addAuthor("Ram Ayall");
    }
    public static void addContent(Document document, List<EBook> dataObjList) throws DocumentException {
        Paragraph paragraph = new Paragraph();
        paragraph.setFont(NORMAL_FONT);
        createReportTable(paragraph, dataObjList);
        document.add(paragraph);
    }
    private static void createReportTable(Paragraph paragraph, List<EBook> dataObjList)
            throws BadElementException {
        PdfPTable table = new PdfPTable(4);
        table.setWidthPercentage(100);
        paragraph.add(new Chunk("Email Report Table :- ", SMALL_BOLD));
        if(null == dataObjList){
        ...
        int count = 1;
        for(EBook dataObject : dataObjList){
    ...
    public static void addTitlePage(Document document, String title) throws DocumentException {
        ...
        preface.add(new Phrase("Recipetory App", PDFCreator.NORMAL_FONT));
